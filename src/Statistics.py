...
class Statistics:
    ...
    profits = [[0 for x in range(7)] for y in range(
        InputsConfig.Runs * len(InputsConfig.NODES))]
    index = 0
    chain = []
    transactions = []
    ...
    def calculate():
        ...
        # gather transaction data from the gateway blockchains
    ...
    def print_to_excel(fname):
        ...
        df1 = pd.DataFrame({'Block Time': [InputsConfig.Binterval], 'Block Propagation Delay': [
                           InputsConfig.Bdelay], 'No. Miners': [len(InputsConfig.NODES)], 'Simulation Time': [InputsConfig.simTime]})
    ...
    def reset2():
        ...
        Statistics.profits = [[0 for x in range(7)] for y in range(
            InputsConfig.Runs * len(InputsConfig.NODES))]  # rows number of miners * number of runs, columns =7
        Statistics.index = 0
        Statistics.chain = []
