...
class BlockCommit(BaseBlockCommit):
    ...
    def receive_block (event):
        ...
        if blockPrev == lastBlockId:
            ...
            if InputsConfig.hasTrans and InputsConfig.Ttechnique == "Full": BaseBlockCommit.update_transactionsPool(node, event.block)
         ...
         #### case 2: the received block is  not built on top of the last block ####
            ...
            if InputsConfig.hasUncles: BlockCommit.update_unclechain(node)
            if InputsConfig.hasTrans and InputsConfig.Ttechnique == "Full": BaseBlockCommit.update_transactionsPool(node,event.block) # not sure yet.
        ...
        while (i < depth):
            if (i < len(node.blockchain)):
                if (node.blockchain[i].id != miner.blockchain[i].id): # and (self.node.blockchain[i-1].id == Miner.blockchain[i].previous) and (i>=1):
                    ...
                    if InputsConfig.hasTrans and InputsConfig.Ttechnique == "Full": BaseBlockCommit.update_transactionsPool(node,newBlock)
            else:
                newBlock = miner.blockchain[i]
